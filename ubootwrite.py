...
def memwrite(ser, path, size, start_addr, verbose, debug, shell):
        ...
        if (bytes_read != size):
                ...
                print("\rProgress 100%                            ")
                writecommand(ser, "bootm {0:08x}".format(start_addr), prompt, verbose)
                return True
...
def main():
        ...
        optparser.add_option("--addr", dest = "addr", help = "mem address", default = "0x82000000", metavar = "addr")
        ...
        if debug:
                ...
                writecommand(ser, "mw 82000000 01234567", prompt, options.verbose)
                buf = ser.read(256)
                print("buf = '" + buf + "'")
                return
